...
public class FibonacciObfuscated extends JFrame {
  ...
  private void initComponents() {
    ...
    this.jSlider1.addChangeListener(new ChangeListener() {
          public void stateChanged(ChangeEvent evt) {
            FibonacciObfuscated.this.changeGeneration(evt);
          }
        });
    ...
    this.GenerateButton.addActionListener(new ActionListener() {
          public void actionPerformed(ActionEvent evt) {
            FibonacciObfuscated.this.buttonPressed(evt);
          }
        });
  ...
  private void asdasdsad() {
    setCursor(Cursor.getPredefinedCursor(3));
    köaskdölasd();
    for (int i = 1; i < this.jSlider1.getModel().getValue(); i++) {
      if (this.recursively.isSelected()) {
        this.Results.append("In generation " + i + " there are " + rhksog(i) + " rabbits.\n");
      } else {
        this.Results.append("In generation " + i + " there are " + asdfasdfsadffasfsad(i) + " rabbits.\n");
      } 
    } 
    setCursor(Cursor.getDefaultCursor());
  }
  
  private long rhksog(int n) {
    if (n <= 1)
      return n; 
    return rhksog(n - 1) + rhksog(n - 2);
  }
  
  private void köaskdölasd() {
  ...
  private long asdfasdfsadffasfsad(int herttjrtzhtr) {
    long j = 0;
    for (int i = 0; i <= herttjrtzhtr; i++)
      j = rzsretsert(i); 
    return j;
  }
  
  public static long rzsretsert(long n) {
    if (n == 0L)
      return 0L; 
    if (n == 1L)
      return 1L; 
